...
def get_mine_positions(data: Data) -> List:
  ...
  squares = data.shape.m  * data.shape.n
  ...
  for count in range(mine_count):
    i = randint(0, data.shape.m)
    j = randint(0, data.shape.n)
    mines.append([i, j])
  return mines
  ...
  for i in range(data.shape.m):
    for j in range(data.shape.n):
      ...
      adjacent_positions = get_adjacent_positions(
        width_upper_bound=data.shape.m, 
        length_upper_bound=data.shape.n, 
        position=position,
      )
...
def access_a_move(data: Data, move: List[int]) -> Data:
  ...
  square to the players score and available adjacent squares.
  '''
